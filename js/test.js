...
var margin = {top: 0, right: 75, bottom: 20, left: 50},
    width = 960,
    height = 500,
    selectX = d3.select("#histograms").append("div")
                .append("span")
                .attr("id", "xvcontainer")
    ...
    min = 3,
    max = 15,
    slcontainer = d3.select("#histograms").select("div").append("span").attr("class", "col-sm-3 col-md-3 col-lg-3").attr("id", "slider"),
    slider =  slcontainer.append("input").attr("type", "range")
                        .property("min", min)
                        .property("max", max)
                        .property("step", 1)
                        .property("defaultValue", 5)
                        .attr("id", "slider"),
    schOnlyBox =     d3.select("#histograms").select("div")
                        .append("span")
                        .attr("class", "col-xs-12 col-md-3")
                        .attr("id", "dfiltername")
                        .text("Show Schools Only? "),
    distfilter = schOnlyBox.append("input")
                            .attr("id", "distfilter")
                            .attr("type", "checkbox")
                            .attr("value", "dfilter")
                            .property("checked", true),
    displayOpt = d3.select("#histograms").select("div")
                    .append("span")
                    .attr("class", "col-xs-12 col-md-3")
                    .attr("id", "pctbox")
                    .text("Display as Frequency?")
                    .append("input")
                    .attr("id", "pctcount")
                    .attr("type", "checkbox")
                    .property("checked", false),
    xvar, filt, dataSet = [], histogram, data, svg;
...
d3.json("data/msas.json", function(error, data) {
    ...
    slider.on("change", function () { histoChart() }),
    ...
    displayOpt.on("change", function() { histoChart() });
...
function histoChart() {
    ...
    tmp.forEach(function(d) { if (!isNaN(+d[xvar])) {
        ...
        } else if (filt && d["schnm"] === "District Level") {

            // Push a null value
            return(dataSet.push(null));
    ...
    svg = d3.select("body")
            .append("div").attr("class", "col-xs-1 col-md-7")
            .append("svg")
